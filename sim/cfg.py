...
if cfg.cellsrec == 0:  cfg.recordCells = cfg.allpops # record all cells
    ...
    for metype in cfg.cellParamLabels:
        if cfg.Nmorpho[metype] < 5:
            for numberME in range(cfg.Nmorpho[metype]):
                cfg.recordCells.append((metype,numberME))
        else:
            numberME = 0
            diference = cfg.Nmorpho[metype] - 5.0*int(cfg.Nmorpho[metype]/5.0)
            
            for number in range(5):            
                cfg.recordCells.append((metype,numberME))
                
                if number < diference:              
                    numberME+=int(np.ceil(cfg.Nmorpho[metype]/5.0))  
                else:
                    numberME+=int(cfg.Nmorpho[metype]/5.0)
...
# Saving
#------------------------------------------------------------------------------
cfg.simLabel = 'v122_batch0'       #   + str(cfg.cynradNumber)
